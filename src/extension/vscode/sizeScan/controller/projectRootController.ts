...
export class ProjectRootController {
	private currentRoot: ScanRoot | undefined;
	...
	initializeFromActiveEditor(): void {
		...
		if (!editor) {
			...
			this.currentRoot = ScanRoot.fromPath(vscode.workspace.workspaceFolders?.[0]?.uri.fsPath);
			return;
		}

		this.currentRoot = ScanRoot.fromPath(this.getRootForEditor(editor));
	}
	...
	handleEditorChange(editor: vscode.TextEditor, debounceMs: number): void {
		const newRoot = ScanRoot.fromPath(this.getRootForEditor(editor));

		if (!newRoot || this.currentRoot?.equals(newRoot)) return;
		...
		this.debounceTimer = setTimeout(() => {
			...
			this.options.onRootChanged(newRoot.path);
		}, debounceMs);
	}
